...
fn get_inbetween_points(c: &mut Context, p1: &(isize, isize), p2: &(isize, isize)) -> Vec<(isize, isize)> {
    ...
    if let Some(data) = c.inbetween_points_cache.get(id as usize) {
        return data.clone()
    }
    ...
    c.inbetween_points_cache[id as usize] = data.clone();
    data
}
...
struct Context {
    inbetween_points_cache: [Vec<(isize, isize)>; (SIZE.pow(4)) as usize],
}

fn init_context() -> Context {
    const EMPTY_VEC: Vec<(isize, isize)> = vec![];
    Context {
        inbetween_points_cache: [EMPTY_VEC; (SIZE.pow(4)) as usize],
    }
}
